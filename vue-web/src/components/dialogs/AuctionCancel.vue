  ...
  >
    ...
    <div class="fg-dialog-body" v-if="!confirm">
      <div class="process">
        <div class="step-info">
          <div class="text">
            <span>{{ $t("dialog.cancelAuction") }}</span>
          </div>
        </div>
        <div class="confirm-text">
          {{ $t("dialog.sureCancelAuction") }}
        </div>
...
export default {
  ...
  methods: {
    ...
    async cancelSale() {
      ...
      buyValue = buyValue.multipliedBy(new BigNumber(10).exponentiatedBy(this.auction.paytokenDecimals))
      var exchangeAddress = this.config.contract.auctionExchangeAddress;
      let order = {
