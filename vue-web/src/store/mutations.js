...
export default {
  ...
  NFT_BALANCE(state, payload) {
    state.nftBalance = Object.assign({}, state.nftBalance, payload);
  },
  ...
  BLIND_PAYTOKENS(state, payTokens) {
    let _paytokens = [];
    for (var i = 0; i < payTokens.length; i++) {
      let paytoken = payTokens[i];
      let metadata = {};
      let symbol = paytoken.symbol;
      if (
        sdk.keyAssetType(paytoken.type) == "ERC721" ||
        sdk.keyAssetType(paytoken.type) == "ERC1155"
      ) {
        metadata = tools.parseMetaData(paytoken.metadataContent);
        symbol = sdk.keyAssetType(paytoken.type);
      }
      _paytokens.push({
        ...paytoken,
    ...
    state.blindPayTokens = _paytokens;
  },
  ...
  BLINDBOX_SIGNED(state, param) {
    state.blindBoxSigned = param;
  },
};
