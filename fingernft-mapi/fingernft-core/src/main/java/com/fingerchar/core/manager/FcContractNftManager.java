...
public class FcContractNftManager {
    ...
    public FcContractNft create(NftInfo nft){
        ...
        contractNft.setType(nft.getType());
    ...
    public void auction(FcAuctionOrder order){
        this.nftItemsManager.auction(order);
        this.auctionOrderManager.auction(order);
    }

    public void auctionBid(FcAuctionOrder order, FcAuctionBids bid){
        this.nftItemsManager.auctionBid(order, bid);
        this.auctionOrderManager.auctionBid(order, bid);
    }
    ...
    public Integer addRoyalties(
            String address,
            String tokenId,
            String rolalties
    ){
        FcContractNft nft = this.getActive(address, tokenId);
        if(null == nft){
            return 0;
        }
        nft.setRoyalties(rolalties);
        return this.update(nft);
    }

    public Integer transfer(TransferLog log, Integer type){
        ...
        if(null == contract ){
            contractManager.add(log.getAddress(), type);
        }
        ...
        BigInteger leftQuanlity = null;
        if(log.getFrom().equalsIgnoreCase(SysConfConstant.ZERO_ADDRESS)){
            // mint transfer
            leftQuanlity = new BigInteger(nft.getQuantity()).add(amount);
            nft.setQuantity(leftQuanlity.toString());
        }else if(log.getTo().equalsIgnoreCase(SysConfConstant.ZERO_ADDRESS)){
            // burn transfer
            leftQuanlity = new BigInteger(nft.getQuantity()).subtract(amount);
            nft.setQuantity(leftQuanlity.toString());
    ...
    private FcContractNft getMedia(FcContractNft nft) {
        ...
        if(StringUtils.isEmpty(nft.getMetadataUrl())){
            if(nft.getType().equals(ContractType.ERC1155.getType())){
                uri = DappWeb3jUtil.getErc1155Uri(nft.getAddress(), nft.getTokenId());
            }else if(nft.getType().equals(ContractType.ERC721.getType())){
    ...
    public FcContractNft add(TransferLog log, Integer type){
    ...
    public List<FcContractNft> getByAddress(String token, String token_id) {
        QueryWrapper<FcContractNft> wrapper = new QueryWrapper<>();
        wrapper.eq(FcContractNft.ADDRESS, token)
                .gt(FcContractNft.TOKEN_ID, token_id)
