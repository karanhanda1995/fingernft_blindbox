...
public class DappWeb3jUtil {
    ...
    public static final String ERC721_INTERFACE_CODE = "0x80ac58cd";

    public static final String ERC1155_INTERFACE_CODE = "0xd9b67a26";
    ...
    public static boolean isErc721(String address) {
        ERC721 erc721 = ERC721.load(address, web3j, new ReadonlyTransactionManager(web3j, address),
                new DefaultGasProvider());
        try {
            return erc721.supportsInterface(Numeric.hexStringToByteArray(ERC721_INTERFACE_CODE)).send();
        } catch (Exception e) {
            return false;
        }
    }

    public static boolean isErc1155(String address) {
        ERC1155 erc1155 = ERC1155.load(address, web3j, new ReadonlyTransactionManager(web3j, address),
                new DefaultGasProvider());
        try {
            return erc1155.supportsInterface(Numeric.hexStringToByteArray(ERC1155_INTERFACE_CODE)).send();
        } catch (Exception e) {
            return false;
        }
    }

    public static ERCTokenInfo getErc1155URI(String address, BigInteger tokenId) throws InterruptedException, ExecutionException, IOException {
        TransactionManager transactionManager = new ClientTransactionManager(web3j, address);
        ERC1155 contract1155 = ERC1155.load(address, web3j, transactionManager, new DefaultGasProvider());
        String uri = contract1155.uri(tokenId).sendAsync().get();
    ...
    public static ERCTokenInfo processUri(String uri) throws IOException {
        ...
        String content = null;
        if (uri.toLowerCase().startsWith("ipfs://ipfs/")) {
            uri = uri.replace("ipfs://ipfs/", "");
            content = IpfsStorage.getIpfsData(uri);
        } else if (uri.toLowerCase().startsWith("http")) {
            content = HttpUtils.post(uri);
        }
