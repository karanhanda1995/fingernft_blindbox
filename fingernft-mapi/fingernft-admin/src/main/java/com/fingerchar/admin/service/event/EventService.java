...
import com.fingerchar.admin.service.FcBlockInfoService;
...
public class EventService extends com.fingerchar.admin.service.event.baseEventService {
    ...
    @Autowired
    BlindBoxEventService blindBoxEventService;

    @Autowired
    NftEventService nftEventService;

    @Autowired
    FcBlockInfoManager blockInfoManager;

    @Autowired
    FcTxOrderManager txOrderManager;

    @Autowired
    FcBlockInfoService fcBlockInfoService;
    ...
    private void processTime(Map<BigInteger, EthBlock.Block> blockMap) {
        ...
        this.auctionOrderManager.processTime(time, maxBlock);
    }
    ...
    private List<EventValuesExt> processEvent(List<EthLog.LogResult> allLogs, Map<BigInteger, EthBlock.Block> blockMap) throws Exception {
        ...
        // 处理拍卖日志
        List<EventValuesExt> list1 = this.auctionEventService.processEvent(allLogs, blockMap);
        list.addAll(list1);

        // 处理盲盒日志
        list1 = this.blindBoxEventService.processEvent(allLogs, blockMap);
        list.addAll(list1);

        // 处理nft日志
        list1 = this.nftEventService.processEvent(allLogs, blockMap);
        list.addAll(list1);
