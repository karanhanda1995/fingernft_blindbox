<template>
	<div class="app-container">
		<div class="filter-container">
			...
			<el-button class="filter-item" type="primary" icon="el-icon-edit" @click="handlerBtn('add')" v-permission="['POST /admin/user/addwhite']">{{$t("whiteUser.add")}}</el-button>
		</div>
		<el-table v-loading="listLoading" :data="list" :element-loading-text="$t('util.loadingText')" border>
			...
			<el-table-column align="center" :label="$t('whiteUser.operation')" width="300" class-name="small-padding fixed-width">
				<template slot-scope="scope">
					<el-button type="danger" size="mini" @click="handlerBtn('del', scope.row)" v-permission="['POST /admin/user/deletewhite']">{{$t('whiteUser.delete')}}</el-button>
				</template>
			</el-table-column>
		</el-table>
