<template>
	<div class="app-container">
		...
		<div class="filter-container">
			<el-input v-model="listQuery.name" clearable class="filter-item w-200 mr-10" :placeholder="$t('userData.placeholder1')" />
			<el-input v-model="listQuery.address" clearable class="filter-item w-200 mr-10" :placeholder="$t('userData.placeholder2')" />
			<el-select v-model="listQuery.verify" class="filter-item w-200 mr-10" :placeholder="$t('userData.placeholder3')">
      ...
      <el-date-picker class="picker" v-model="value1" type="daterange" range-separator="至"
        start-placeholder="开始日期" end-placeholder="结束日期"></el-date-picker> -->
			...
			<el-button class="filter-item mt-10" type="primary" icon="el-icon-search" @click="handleFilter">{{ $t("userData.select") }}</el-button>
		</div>
		...
		<el-table v-loading="listLoading" :data="list" :element-loading-text="$t('global.loadingText')" border fit highlight-current-row>
			...
			<el-table-column align="center" :label="$t('userData.avatar')" prop="avatar">
				<template slot-scope="scope" v-if="scope.row.avatar">
					<media :isPreview="true" :url="scope.row.avatar" type="image"></media>
				</template>
			</el-table-column>
			<el-table-column align="center" :label="$t('userData.certificationStatus')" prop="userVerify">
				<template slot-scope="scope">
					<el-tag v-if="scope.row.userVerify == 0 || !scope.row.userVerify" type="info">{{ $t("userData.unauthenticated") }}</el-tag>
			...
			<el-table-column align="center" :label="$t('userData.operation')" width="250" class-name="small-padding fixed-width">
				<template slot-scope="scope">
					<el-button type="primary" size="mini" @click="handleDetail(scope.row)">{{ $t("userData.details") }}</el-button>
					<el-button type="success" size="mini" class="widthFit" v-permission="['POST /admin/user/verify']" @click="(verifyForm = true), (activeId = scope.row.id)" v-if="scope.row.userVerify != 1">
						{{ $t("userData.immediateCertification") }}</el-button>
					<el-button type="success" size="mini" class="widthFit" v-permission="['POST /admin/user/verify']" @click="handleCertification(scope.row)" v-else>{{ $t("userData.cancelCertification") }}
					</el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		<el-dialog :title="$t('userData.details')" :visible.sync="dialogFormVisible">
			<el-descriptions class="margin-top" direction="vertical" :column="3" size="" border>
		...
		<el-dialog :title="$t('userData.authentication')" :visible.sync="verifyForm">
