    ...
    <el-form-item :label="label" :prop="addressKey" required>
      <div style="display: flex;">
        ...
        <div style="padding-left: 15px;">
          <template v-if="editKey === propKey">
            <el-button v-permission="['*']" size="mini" :loading="loading" plain class="mr-10" type="primary" @click="updateConfig(1)">{{ $t('global.update') }}</el-button>
            <el-button size="mini" :loading="loading" plain @click="$emit('setEditKey', '')">{{ $t('global.cancel') }}</el-button>
          </template>
          <template v-else>
            <el-button v-permission="['*']" size="mini" :loading="loading" plain class="mr-10" type="danger" @click="editConfig">{{ $t('global.edit') }}</el-button>
            <el-button size="mini" :loading="loading" plain type="success" v-permission="['POST /admin/config/update']" @click="deploy">{{ $t('global.deploy') }}</el-button>
          </template>
        </div>
      </div>
    </el-form-item>
