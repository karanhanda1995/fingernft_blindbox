<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.nftVerify" class="filter-item w-200 mr-10" :placeholder="$t('nftManagement.placeholder4')" :clearable="true">
        <el-option v-for="item in nftVerifys" :key="item.value" :label="item.label" :value="item.value" />
      </el-select>
    ...
    <el-table v-loading="listLoading" :data="list" :element-loading-text="$t('nftManagement.loadingText')" border fit highlight-current-row>
      ...
      <el-table-column align="center" :label="$t('nftManagement.authenticationStatus')" prop="nftVerify" width="150">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" :label="$t('nftManagement.nftAddress')" prop="address" />
      <el-table-column align="center" :label="$t('nftManagement.creator')" prop="creator" />
      ...
      <el-table-column align="center" :label="$t('nftManagement.operation')" width="300" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="handleDetail(scope.row)">{{$t('nftManagement.details')}}</el-button>
          <el-button v-permission="['POST /admin/nft/verify']" type="success" size="mini" @click="verifyForm=true,selectNft=scope.row" v-if="scope.row.nftVerify!=1">
            {{$t('nftManagement.immediateCertification')}}</el-button>
          <el-button v-permission="['POST /admin/nft/verify']" type="success" size="mini" @click="cancelCertification(scope.row)" v-else>{{$t('nftManagement.cancelCertification')}}
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      verifyFunction () {
        this.listLoading = true;
        this.$api("nft.verify", { address: this.selectNft.address, tokenId: this.selectNft.tokenId, nftVerify: this.verifyStatus })
          .then((response) => {
            this.$notify.success({
      ...
      cancelCertification (row) {
        ...
        this.$confirm(this.$t('util.tipContent1'), this.$t('util.tip'), {
          ...
          .then(() => {
            that.$api("nft.verify", { address: row.address, tokenId: row.tokenId, nftVerify: 0 })
              .then((response) => {
